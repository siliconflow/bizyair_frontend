...
test.describe('Move Node', () => {
  ...
  ;[
    ...
    test(nodeName, async ({ page }) => {
      await page.goto(
        `http://localhost:${defaultComfyPort + parseInt(process.env.TEST_PARALLEL_INDEX)}/`
      )
      ...
      const input = page.locator('.comfy-vue-node-search-container input[type="text"]')
      const dropdown = page.locator('.comfy-vue-node-search-container .p-autocomplete-list')
      await expect(async () => {
