      ...
      <TableBody>
        ...
        <template v-else>
          <template v-if="!isLoading && modelStoreInstance.models.length > 0">
            <template
              v-for="model in modelStoreInstance.models"
              :key="`${model.id}-${model.name}`"
            >
              <TableRow
                class="group cursor-pointer border-[#F9FAFB]/60 hover:bg-transparent h-12"
              >
                <TableCell class="w-[55%]" @click="toggleExpand(model.name)">
                  <div class="flex items-center space-x-2">
                    <span class="text-sm">
                      <svg
                        v-if="expandedModels.has(model.name)"
                ...
                <TableCell class="w-[15%]">
                  <div class="flex justify-end h-full">
                    <Popover
                      ...
                      "
                      class="bg-[#353535] z-[5100]mmm"
                      :open="currentOperateModel === model.name"
                      @update:open="
                        (value: any) =>
                          value
                            ? (currentOperateModel = model.name)
                            : (currentOperateModel = '')
                      "
                    >
                      <PopoverTrigger class="bg-transparent">
                        <div
                          class="flex justify-center items-center hover:bg-[#222222] rounded-md w-8 h-8"
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="24"
                        ...
                        <Command>
                          <CommandList>
                            <CommandGroup>
                              <CommandItem
                                value="edit"
                                @click="handleOperateChange('edit', model)"
                              ...
                              <CommandItem
                                value="remove"
                                @click="handleOperateChange('remove', model)"
          ...
          <template v-else>
            <TableRow class="hover:bg-transparent">
              <TableCell colspan="4" class="h-[400px]">
                <div
                  class="flex items-center justify-center h-full text-[#F9FAFB]"
                >
                  No available data
                </div>
              </TableCell>
