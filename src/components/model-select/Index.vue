...
const getModelList = async () => {
  ...
  await get_model_list(
    modelStoreInstance.modelListPathParams,
    modelStoreInstance.filterState,
  )
    .then((response) => {
      if (response && response.data) {
        modelStoreInstance.modelListPathParams.total =
  ...
  return model_types()
    .then((modelTypesResponse) => {
      modelStoreInstance.setModelTypes(
        modelTypesResponse?.data
          ? (modelTypesResponse.data as CommonModelType[])
          : [],
      );
    ...
    .then((baseModelResponse) => {
      modelStoreInstance.setBaseModelTypes(
        baseModelResponse?.data
          ? (baseModelResponse.data as CommonModelType[])
          : [],
      );
    })
    .catch((error) => {
...
watch(
  () => modelStoreInstance.applyObject,
...
watch(
  () => modelStoreInstance.closeModelSelectDialog,
...
watch(
  () => modelStoreInstance.modelListPathParams.current,
  async (newVal: number, oldVal: number) => {
...
<template>
  <v-dialog
    ...
    class="max-w-[70%] px-6 pb-6 overflow-hidden"
    @onClose="handleClose"
    contentClass="custom-scrollbar max-h-[80vh] overflow-y-auto w-full rounded-tl-lg rounded-tr-lg custom-shadow"
  >
    <template #title>
      <span
        class="text-[#F9FAFB] mb-4 text-[18px] font-semibold leading-[18px] tracking-[-0.45px]"
      >
        Select Model
      </span>
    </template>
    ...
    <div class="font-['Inter'] flex flex-col">
      ...
      >
        <TabsList
          class="grid w-full grid-cols-3 h-12 bg-[#4E4E4E] text-white text-sm shrink-0"
        >
          <TabsTrigger
            v-for="mode in modes"
            :key="mode"
        ...
        <template v-for="mode in modes" :key="mode">
          <TabsContent
            v-if="modelStoreInstance.mode === mode"
            :value="mode"
