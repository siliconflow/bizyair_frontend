  ...
  const fetchData = async (
    ...
    return new Promise(resolve => {
      const doFetch = async () => {
        ...
        if (isPageProvider) {
          communityStore.mainContent.modelListPathParams.current =
    ...
    <div class="flex-1 px-6 relative">
      <div class="scroll-container overflow-y-auto px-2">
        ...
        <div
          v-if="
            !isGridLoading &&
            (!communityStore.mainContent.models || communityStore.mainContent.models.length === 0)
          "
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <img
                      :src="
                        Array.isArray(model.versions?.[0]?.cover_urls)
                          ? model.versions?.[0]?.cover_urls[0]
                          : model.versions?.[0]?.cover_urls
                      "
                      :alt="model.versions?.[0]?.version || model.name"
                      :crossorigin="
                        typeof (Array.isArray(model.versions?.[0]?.cover_urls)
                          ? model.versions?.[0]?.cover_urls[0]
                          : model.versions?.[0]?.cover_urls) === 'string' &&
                        (Array.isArray(model.versions?.[0]?.cover_urls)
                          ? model.versions?.[0]?.cover_urls[0]
                          : model.versions?.[0]?.cover_urls
                        )?.startsWith('blob:')
