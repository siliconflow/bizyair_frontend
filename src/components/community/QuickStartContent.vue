<script setup lang="ts">
  ...
  const generateMockData = (startId: number, count: number): Model[] => {
    return Array.from({ length: count }, (_, index) => ({
      ...
      imageUrl:
        'https://image.civitai.com/xG1nkqKTMzGDvpLrqFT7WA/a85e1a6f-ed98-4b05-8cdf-6eaf419e3439/original=true,quality=90/44285573.jpeg',
      stats: {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (response?.data) {
        ...
        communityStore.quickStart.modelListPathParams.total = 0
        communityStore.quickStart.models = []
        hasMore.value = false
        hasPrevious.value = false
      }
