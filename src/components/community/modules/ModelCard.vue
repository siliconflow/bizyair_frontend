      ...
      <template v-if="!loading && model">
          ...
          <vTooltips :tips="actionTooltipText">
            <div
              ...
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none">
                <path d="M2.91663 1.75L11.0833 7L2.91663 12.25V1.75Z" stroke="#F3F4F6" stroke-linecap="round"
        ...
        <div class="relative aspect-[2/3] md:aspect-[3/4] lg:aspect-[2/3] overflow-hidden"
          ...
          <div class="absolute inset-0 bg-gradient-to-br from-[#2a2a2a] to-[#1a1a1a]"
            :class="{ 'opacity-0': props.imageLoaded }"></div>

          <!-- 视频显示（悬停时） -->
          <video v-if="isVideo && isHovering && currentMediaSrc" :src="currentMediaSrc" controls
            class="absolute inset-0 w-full h-full object-cover transition-all duration-300" :class="{
              'opacity-0': !props.imageLoaded,
              'opacity-100 group-hover:scale-105': props.imageLoaded
            }" muted autoplay loop preload="metadata" @loadeddata="handleImageLoad" @error="handleImageError" />

          <img v-else-if="currentMediaSrc" :src="currentMediaSrc" :alt="model.versions?.[0]?.version || model.name"
            :crossorigin="typeof currentMediaSrc === 'string' && currentMediaSrc.startsWith('blob:')
                ? 'anonymous'
                : undefined
              " class="absolute inset-0 w-full h-full object-cover transition-all duration-300" :class="{
              'opacity-0': !props.imageLoaded,
          ...
          <div v-if="model.tags && model.tags.includes(10005)"
            ...
            <svg width="48" height="49" viewBox="0 0 48 49" fill="none" xmlns="http://www.w3.org/2000/svg"
              ...
              <path
                ...
                fill="#DC2626" />
              <rect x="5.6217" y="19.7309" width="35.8941" height="10.4318" rx="2" fill="url(#pattern0_2578_7109)" />
              <rect x="13.6541" y="4.70111" width="19.8295" height="16" rx="2" fill="url(#pattern1_2578_7109)" />
              <defs>
                <pattern id="pattern0_2578_7109" patternContentUnits="objectBoundingBox" width="1" height="1">
                  <use xlink:href="#image0_2578_7109" transform="matrix(0.00301187 0 0 0.0103633 -0.56617 0)" />
                </pattern>
                <pattern id="pattern1_2578_7109" patternContentUnits="objectBoundingBox" width="1" height="1">
                  <use xlink:href="#image0_2578_7109" transform="scale(0.00545189 0.00675676)" />
                </pattern>
                <image id="image0_2578_7109" width="520" height="148" preserveAspectRatio="none"
        ...
        <div class="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/90 to-black/0">
          <div class="flex items-center gap-2">
            <span v-if="model.tags && model.tags.length > 0 && tagsStore.getHighestOrderTag(model.tags)"
          ...
          <div class="flex items-center space-x-3 text-white/90 text-xs">
            <span v-if="model?.type === 'Workflow' && model?.counter?.downloaded_count"
              class="flex items-center space-x-1">
              <vTooltips :tips="t('community.modelCard.tooltips.downloaded')">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <path data-v-eb6fbda7=""
                    ...
                    stroke="#F9FAFB" stroke-linecap="round" stroke-linejoin="round"></path>
                </svg>
              </vTooltips>
