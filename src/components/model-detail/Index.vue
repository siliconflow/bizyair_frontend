...
const fetchModelDetail = async () => {
  const res = await model_detail({
    id: props.modelId,
    source: modelStoreInstance.mode,
  });
  if (!res.data) {
...
const initializeScroll = () => {
  if (model.value && model.value.versions && model.value.versions.length > 0) {
    if (props.version?.id) {
      const targetVersion = model.value.versions.find(
        (v) => v.id === props.version.id,
      );
      if (targetVersion) {
...
<template>
  ...
  >
    ...
    >
      <div
        class="flex flex-row gap-2 items-center justify-start self-stretch shrink-0 relative"
      >
        <div
      ...
      >
          ...
          <div class="min-w-[200px] max-w-[600px]">
            <ScrollArea ref="scrollViewportRef" class="rounded-md w-full">
              <div class="whitespace-nowrap">
                ...
                >
                  <TabsList
                    class="inline-flex h-12 bg-transparent text-sm text-white w-auto"
                  >
                    <TabsTrigger
        ...
        <div class="flex gap-8">
          ...
          <Popover
            v-if="
              modelStoreInstance.mode === 'my' ||
              modelStoreInstance.mode === 'my_fork'
            "
            class="bg-[#353535]"
            :open="downloadOpen"
          ...
          >
            <PopoverTrigger class="bg-transparent">
              <div
                class="flex justify-center items-center rounded-md w-8 relative z-50"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
      ...
      <div
        class="flex flex-row gap-4 items-start justify-start shrink-0 relative"
      >
        <div
    ...
    <div
      class="flex flex-row gap-8 items-start justify-start self-stretch flex-1 relative"
    >
      <div
      ...
      >
        <div class="w-full min-h-[80vh]">
          <MdPreview
            ...
            id="previewRef"
            :modelValue="currentVersion?.intro"
            :noImgZoomIn="true"
            :preview="true"
            theme="dark"
          ...
          <div
            v-else
            class="w-full h-[80vh] bg-[#353535] rounded-tl-lg rounded-tr-lg"
          >
      ...
      <div
        class="flex flex-col gap-6 items-start justify-start w-[40%] relative"
      >
        <div
        ...
        >
          ...
          >
            <Button
              variant="default"
              v-if="modelStoreInstance.mode === 'publicity'"
        ...
        >
          <div className="flex w-full text-gray-300 text-sm">
            <div
              className="w-[100px] bg-[#4E4E4E80] p-4   border-b border-[rgba(78,78,78,0.50)]"
            >
              Type
            </div>
            <div
              className="flex-1 p-4 border-b text-sm border-[rgba(78,78,78,0.50)]"
            >
              <span
          ...
          <div className="flex w-full">
            ...
            >
              <span>
                {{
                  currentVersion?.sign
                    ? sliceString(currentVersion?.sign, 15)
                    : ""
                }}
              </span>
              <svg
          ...
          <div className="flex w-full">
            ...
            <div
              className="flex-1 p-4 border-b border-[rgba(78,78,78,0.50)] flex flex-row gap-2"
            >
              <div
                class="bg-[#6D28D933] rounded-radius-rounded-xl pr-1.5 pl-1.5 flex flex-row gap-1 items-center justify-center shrink-0 min-w-[40px] relative overflow-hidden"
              >
          ...
          >
            {{
              currentVersion?.file_name
                ? sliceString(currentVersion?.file_name, 20)
                : ""
            }}
