...
<template>
  <Teleport to="body">
    <div
      :class="
        cn(
          'fixed w-[100vw] h-[100vh] bg-black/80 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0',
          props.layoutClass
        )
      "
      v-if="open"
    >
      <div
        :class="
          cn(
            'px-0 overflow-hidden pb-0 max-w-[900px] bg-[#222] fixed left-1/2 top-1/2 grid w-full -translate-x-1/2 -translate-y-1/2 gap-4 border p-6 shadow-lg sm:rounded-lg',
            props.class
          )
        "
      >
