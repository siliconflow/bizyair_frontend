...
export const check_folder = (url: string) =>
  customFetch(
    `/bizyair/modelhost/check_folder?absolute_path=${encodeURIComponent(url)}`,
    { method: "GET" },
  );
...
export const getDescription = (data: any) =>
  customFetch(
    `/bizyair/modelhost/models/description?${new URLSearchParams(data).toString()}`,
    {
      method: "get",
    },
  );
...
export const create_models = (data: any) =>
  customFetch(
    `/bizyair/community/models?clientId=${sessionStorage.getItem("clientId")}`,
    {
      method: "post",
      body: JSON.stringify(data),
    },
...
export const checkLocalFile = (data: any) =>
  customFetch(
    `/bizyair/community/check_local_file?${new URLSearchParams(data).toString()}`,
    {
      method: "get",
    },
  );
...
export const submitUpload = (data: any) =>
  customFetch(
    `/bizyair/community/submit_upload?clientId=${sessionStorage.getItem("clientId")}`,
    {
      method: "POST",
      body: JSON.stringify(data),
    },
...
export const model_types = () =>
  customFetch("/bizyair/community/model_types", { method: "GET" });
...
export const model_detail = (data: any) =>
  customFetch(
    `/bizyair/community/models/${data.id}/detail?source=${data.source}`,
    { method: "GET" },
  );
