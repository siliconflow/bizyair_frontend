  ...
  >
    ...
    <div v-show="modelBox" class="px-6 pb-6">
      ...
      <v-item label="Model Type">
        ...
        >
          <SelectItem v-for="(e, i) in typeLis" :key="i" :value="e.value">{{
            e.label
          }}</SelectItem>
        </v-select>
      </v-item>
    ...
    >
      <AccordionItem
        class="bg-[#353535] z-1 px-6 w-full rounded-tl-lg rounded-tr-lg custom-shadow border-t-[1px]"
        v-for="(e, i) in formData.versions"
        :key="i"
        :value="`${i}`"
      >
        <v-accordion-trigger class="justify-between relative">
          <span v-if="acActiveIndex !== `${i}` && e.version">{{
            e.version
          }}</span>
          ...
          <v-item label="File Path">
            <div class="flex">
              ...
              <Button
                @click="interrupt(e)"
                class="ml-2"
                :disabled="!e.progress || e.progress == 100"
                >interrupt</Button
              >
            </div>
          </v-item>
    ...
    <div
      v-if="showLayoutLoading"
      class="z-50 w-full h-full absolute left-0 top-0"
    ></div>
  </v-dialog>
</template>
...
function verifyVersion() {
  ...
  return tempData.versions.every(
    (e: any) => e.version && e.base_model && e.filePath,
  );
}
...
watch(
  ...
  (val: any) => {
    if (val.type == "progress") {
      ...
      formData.value.versions[i].progress = Number(
        val.data.progress.replace("%", ""),
      );
    }
