<template>
  ...
  <vDrawer v-model:open="showDrawer">
    <div class="bizyair-profile-primary flex items-center">
      ...
      <div v-if="statusStore.infoData.name" class="ml-4">
      ...
      <div class="flex items-center ml-2">
        <span v-show="!isEditingShareId">
          ...
          <vTooltips
            v-else
            :tips="
              generateShareIDMessage(
                statusStore.infoData.last_share_id_update_at,
              )
            "
          >
...
const canEditShareId = computed(() => {
  ...
  return (
    !isNaN(lastUpdate) && Date.now() - lastUpdate > 1000 * 60 * 60 * 24 * 365
  );
});
